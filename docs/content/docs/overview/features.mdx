...
Canvas Agent bridges the `infinite-canvas` MCP tools to the canvas open in your browser:

- Any MCP-capable client (Codex CLI, Claude Code, pi, etc.) can read and operate the active canvas.
- The browser only talks to the local Canvas Agent; no browser state is exposed to a server.
- Multiple tabs are isolated: tool calls always target the most recently activated canvas tab.
- Site-level tools cover the canvas list, Image/Video Studio, prompts, and assets.

See [Canvas Agent](https://github.com/liujiacheng1234/infinite-canvas/blob/main/canvas-agent/README.md).
