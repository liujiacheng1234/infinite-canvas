export default {
    ...
    agent: {
        connection: { title: "本地 Agent 连接", description: "MCP 客户端通过本机 Canvas Agent 读写当前画布；连接后画布顶栏会显示连接状态。", url: "Local URL", token: "Connect Token", connect: "连接", disconnect: "断开" },
        status: { connected: "已连接", connecting: "连接中", disconnected: "未连接" },
        state: { connectionRequired: "请填写 Local URL 和 Connect token", invalidUrl: "Local URL 格式不正确", offline: "离线", requestFailed: "本地 Agent 请求失败" },
        ...
        composer: { mentions: { resourceMissing: "画布素材“{{title}}”已不存在", imageReadFailed: "无法读取画布图片“{{title}}”" } },
        runtime: { agentOutdated: "本地 Agent 版本过旧，请重启 Canvas Agent 后重新连接", restartRequired: "需要重启 Agent", connected: "已连接", connectionLost: "连接断开", connectionFailed: "连接失败", connectionLostDescription: "本地 Agent 连接失败或已断开", connectionFailedDescription: "连接失败，请检查地址和 token", openCanvasFirst: "当前不在画布页，请先用 site_navigate 打开画布", canvasOperationFailed: "画布操作失败", unknownCanvasTool: "不支持的画布工具：{{name}}", opsReceiptHint: "已返回执行回执；需要查看最新画布结构用 canvas_get_state，查看节点内容用 canvas_get_nodes" },
    },
    locale: {
        zhCN: "简体中文",
        enUS: "English",
