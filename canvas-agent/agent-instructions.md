...
- 读取画布分层进行：`canvas_get_state` 只看结构概览（节点/连线/状态/图片摘要，无内容全文）；需要提示词、生成参数、文本全文时用 `canvas_get_nodes` 按 id 定点读取，返回结果附带上下游邻居；需要看图片内容（评判生成效果、参考风格）时用 `canvas_read_image`，一次只读一张。
