...
export const canvasOpSchema = z.discriminatedUnion("type", [
    ...
    z.object({ type: z.literal("delete_connections"), id: z.string().optional(), ids: z.array(z.string()).optional(), fromNodeId: z.string().optional(), toNodeId: z.string().optional(), all: z.boolean().optional() }).passthrough(),
...
export const toolInputSchemas = {
    ...
    canvas_get_nodes: z.object({ ids: z.array(z.string()).min(1).max(20) }),
    canvas_read_image: z.object({ nodeId: z.string(), imageId: z.string().optional() }),
    ...
    canvas_disconnect_nodes: z.object({ connections: z.array(z.object({ fromNodeId: z.string(), toNodeId: z.string() })).min(1) }),
...
export const toolDescriptions: Record<ToolName, string> = {
    ...
    canvas_get_state: "读取当前网页画布的结构概览，返回表头行格式文本（非 JSON）：canvas 行含 projectId 和标题；nodes 表列为 i,id,type,title,x,y,w,h,mode,status,imgs,error，id 为短引用（n1、n2…），后续所有工具的节点入参直接用短引用（也接受真实节点 id），status 为 - 表示 idle/success，imgs 为 数量:主图宽x高，含逗号的字段用 CSV 引号转义；connections 表的 from/to 是节点行号 i（仅本次返回内有效）。不包含节点内容全文；需要提示词、生成参数全文时用 canvas_get_nodes，需要看图时用 canvas_read_image。",
    canvas_get_selection: "读取当前网页画布选中的节点，返回与 canvas_get_state 相同列格式的 nodes 行表（id 为短引用）。",
    canvas_get_nodes: "按 id 读取 1~20 个节点的完整 metadata：文本全文、生成参数、texts 备选、errorDetails，并附带每个节点的上下游邻居（删除或改连线前先查看影响）。id 可用短引用或真实节点 id，返回的 id 也是短引用。返回 missing 列出未找到的 id。",
    canvas_read_image: "读取图片节点的一张图片并返回图像内容，用于评判生成效果或参考风格。默认主图，可用 imageId 指定备选图；一次只读一张，仅在需要看图时调用。",
    canvas_apply_ops: "批量操作当前网页画布。ops 支持 add_node、update_node、delete_node、delete_connections（支持 fromNodeId/toNodeId 过滤）、connect_nodes、set_viewport、select_nodes、run_generation。节点引用入参可用短引用（n1、n2…）或真实 id。返回每个 op 的执行回执（成功/失败/原因）、新建节点短引用 id 和被断开的连线，不再返回全量快照；需要查看结果时用 canvas_get_state 或 canvas_get_nodes。",
    ...
    canvas_disconnect_nodes: "按 from/to 批量断开节点连线，删除或重连前可先用 canvas_get_nodes 查看上下游。",
